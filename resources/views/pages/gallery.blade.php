...
<x-section title="Image and Video Gallery"><div class="grid gap-5 md:grid-cols-3">@foreach(\App\Support\TrinetraData::galleryCategories() as $i=>$category)<figure class="overflow-hidden rounded border border-slate-200 bg-white"><div class="aspect-video"><img src="{{ asset(array_values($images)[$i % count($images)]) }}" alt="{{ $category }} transport visual" class="h-full w-full object-cover"></div><figcaption class="p-4 text-sm text-steel">{{ $category }}. Replace with owned project media when available.</figcaption></figure>@endforeach</div></x-section>
@endsection
